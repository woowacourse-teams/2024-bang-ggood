...
public class Questionlist {
    ...
    public static boolean contains(int questionId) {
        return questions.containsKey(questionId);
    }

    public static boolean isQuestionsInCategory(int questionId, int categoryId) {
        if (!questions.containsKey(questionId)) {
            throw new BangggoodException(ExceptionCode.INVALID_QUESTION);
        }
        if (!Category.contains(categoryId)) {
            throw new BangggoodException(ExceptionCode.CATEGORY_NOT_FOUND);
        }

        return questions.get(questionId).getCategoryId() == categoryId;
    }

    public static String getTitleByQuestionId(int questionId) {
        return questions.get(questionId).getTitle();
    }

    public static String getSubtitleByQuestionId(int questionId) {
    ...
    public static int getQuestionIdByQuestion(Question question) {
        return questions.entrySet().stream()
                .filter(entry -> entry.getValue().equals(question))
                .map(Map.Entry::getKey)
                .findFirst()
                .orElseThrow(() -> new BangggoodException(ExceptionCode.INVALID_QUESTION));
    }

    private static void initClean() {
    ...
    private static void initRoomCondition() {
    ...
    private static void initEconomic() {
