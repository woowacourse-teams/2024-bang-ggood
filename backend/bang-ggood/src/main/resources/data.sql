...
INSERT INTO question (id, category_id, title, subtitle, is_default)
VALUES
    (1, 1, '곰팡이가 핀 곳 없이 깨끗한가요?', '천장, 벽면, 가구 뒤, 장판을 확인하세요.', true),
    (2, 1, '불쾌한 냄새 없이 쾌적한가요?', null, true),
    (3, 1, '벌레가 나온 흔적 없이 깔끔한가요?', '벌레 퇴치약이 부착되어 있는지 확인하세요.', true),
    (4, 1, '물건을 충분히 수납할 수 있는 공간이 있나요?', null, true),
    (5, 1, '방 인테리어는 괜찮나요?', null, true),
    (6, 1, '에어컨의 상태는 괜찮은가요?', '에어컨을 틀어서 불쾌한 냄새가 나진 않는지 확인하세요.', false),
    (7, 1, '보일러가 잘 동작하나요?', null, false),
    (8, 1, '콘센트 위치와 개수가 적절한가요?', null, false),
    (9, 1, '벽지 상태가 양호한가요?', null, false),

    (10, 2, '창 밖의 뷰가 가로막힘 없이 트여있나요?', null, true),
    (11, 2, '창문 상태가 괜찮나요?', null, true),
    (12, 2, '환기가 잘 되는 구조인가요?', '창문 크기와 방향을 확인하세요.', true),
    (13, 2, '햇빛이 잘 들어오나요?', null, true),
    (14, 2, '창문이 이중창인가요?', null, false),
    (15, 2, '창문 밖에 쓰레기통 등 냄새가 나는 요소가 있나요?', null, false),

    (16, 3, '화장실이 깨끗한가요?', '청소 가능한 얼룩인지 확인하세요.', true),
    (17, 3, '수압 및 물 빠짐이 괜찮은가요?', '화장실에서 수도와 변기를 동시에 사용해보세요.', true),
    (18, 3, '환기 시설이 있나요?', null, true),
    (19, 3, '내부에 창문이 있나요?', null, false),
    (20, 3, '온수가 잘 나오나요?', null, false),

    (21, 4, '잠금장치가 있는 공동 현관문이 있나요?', null, true),
    (22, 4, '출입구와 복도에 CCTV가 설치되어 있나요?', null, true),
    (23, 4, '관리자분이 함께 상주하시나요?', '관리자분이 24시간 상주하시는지 확인하세요.', true),
    (24, 4, '보안 시설이 잘 갖추어져 있나요?', '도어락, 창문 잠금장치 등이 있는지 확인하세요.', false),
    (25, 4, '화면이 달린 인터폰이 제공되나요?', null, false),
    (26, 4, '현관문에 걸쇠가 있나요?', null, false),

    (27, 5, '주변 도로가 밤에도 충분히 밝은가요?', null, false),
    (28, 5, '주변에 소음 시설이 있는지 확인했나요?', '유흥시설, 놀이터, 공사장이 있는지 확인하세요.', false),
    (29, 5, '1층에 음식점이 있는지 확인했나요?', null, false),
    (30, 5, '집 가는 길이 언덕 없이 완만한가요?', null, false),
    (31, 5, '옆 건물에서 보이는 구조인지 확인했나요?', null, false),
    (32, 5, '주차할 수 있는 시설이 있나요?', null, false)
;

INSERT INTO highlight (id, question_id, name)
VALUES
    (1, 1, '곰팡이'),
    (2, 2, '불쾌한 냄새'),
    (3, 3, '벌레'),
