...
/**
 * useHandleTip 팁박스를 보이거나 / 숨기게 해주는 로직을 관리하는 훅입니다.
 * @param tipType : 팁의 타입, 이름을 나타냅니다.
 * @returns
 * closedTips 은 팁박스를 숨기는 함수입니다.
 * isTipOpen 는 해당 팁박스가 숨겨져 있는지 알려주는 boolean 입니다.
 * resetShowTip 는 다시 팁박스를 보이게 해주는 함수입니다.
 */

const useHandleTip = (tipType: TipType) => {
  ...
  return { isTipOpen: getCurrentTipIsOpen(), closeTip, resetShowTip };
};

export default useHandleTip;
